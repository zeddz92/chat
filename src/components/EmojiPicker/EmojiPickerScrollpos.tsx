...
export const EmojiPicker: FC<EmojiPickerProps> = ({ variant = "default" }) => {
  ...
  // const [recentEmojis] = useLocalStorage("emoji-picker-recent", []);
  ...
  return (
    <div
      className="text-white w-full flex flex-col bg-primary-600 overflow-hidden"
      style={{ height: 300 }}
    >
      <CustomTabs
        variant={variant}
      ...
      >
        ...
        <div className="w-full px-3 my-2 relative">
          {resultEmojis && (
            <div
              style={{ height: 244 }}
              className="pt-12.5 relative overflow-y-scroll overflow-x-hidden"
            >
              <div className="grid grid-cols-flow gap-y-3 pr-4 relative">
              ...
              {groupedEmojis &&
                categories.map(({ id, name }, index) => (
                  <div key={id} className="mb-6">
                    ...
                    <div data-category={id} className="emoji-category-bottom" />
                  </div>
                ))}
            </div>
          </div>
